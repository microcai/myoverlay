...
src_compile() {
    ...
    if use x86; then
        eautoreconf 
        econf --prefix=${_prefix} --enable-libpath=${_libdir} LDFLAGS="-L$(pwd)/../com/libs_bin32" || die "Error: scangearmp/autoconf.sh failed"
    elif use amd64 ; then
        eautoreconf
	econf --prefix=${_prefix} --enable-libpath=${_libdir} LDFLAGS="-L$(pwd)/../com/libs_bin64" || die "Error: scangearmp/autoconf.sh failed"
    else
	die "Not supported platform"
    fi

    emake || die "Couldn't make scangearmp"
